...
def render(
    ...
    # Show active mode inside the widget (LLM vs offline fallback)
    mode = (
        "LLM"
        if (os.getenv("ANALYST_ENABLED", "1") not in ("0", "false", "False")
    ...
    q = st.text_area(
        ...
        key=f"analyst_q_{ticker}",  # avoid cross-ticker state collisions
    )
    col1, col2 = st.columns([1, 4])
    with col1:
        go = st.button("Ask", use_container_width=True, key=f"analyst_go_{ticker}")
